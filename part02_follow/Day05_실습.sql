...
-- emp 테이블에 제약 사항 추가 > primary key(emp_id) 기본키 제약 추가
...
-- emp 테이블에 제약 사항 추가 > foreign key(dept_id) 참조키 제약 추가
...
-- emp 테이블에 제약 사항 추가 > foreign key(dept_id) 참조키 제약 추가 > 고소해 사원 부서이동 --> ACC
...
-- emp 테이블에 제약 사항 추가 > foreign key(dept_id) 참조키 제약 추가 > 홍길동 사원 추가
