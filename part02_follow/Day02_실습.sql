...
-- (4) upper(문자열), lower(문자열) : 대문자, 소문자로 치환 / 오라클에서는 뒤에 케이스가 붙음, uppercase, lowercase
	...
	-- ㄴ 영어이름은 전체 대문자, 부서아이디는 소문자, 이메일은 대문자
...
-- (6) format(문자열, 소수점자리) : 문자열 포맷
	...
	-- ㄴ 급여, 보너스는 소수점 없이 3자리 콤마(,)로 구분하여 출력
...
-- ㄴ  입사일, 퇴사일 출력은 '-'을 '/'로 치환하여 출력
-- ㄴ 재직중인 사원은 현재 날짜를 출력
-- ㄴ 급여 출력시 3자리 콤마(,) 구분
...
select * from employee limit 3; -- mysql에만 limit 개념이 있고 다른 db에는 리미트가 없어 다른 방식으로 처리
